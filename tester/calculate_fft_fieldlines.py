...
class MagneticFieldLineAnalyser:
    ...
    as a function of the toroidal angle £ and applies a Gaussian-windowed
    ...
    def calculate_fourier_transform(self, coordinate_vals, omega_range, lambda_width):
        ...
        for omega in omega_range:
            ...
            integral = np.trapezoid(integrand, self.zeta)
    ...
    def find_modes(self, coordinate='Z', omega_min=-1.0, omega_max=1.0, num_omega=4000, lambda_width=50.0):
        ...
        Args:
            ...
            lambda_width (float): Gaussian window width (λ).  -> Key variable for permutation
...
# 2. CALCULATING R(ζ) AND Z(ζ) DATA
def flux_to_cylindrical(file_path):
    ...
    try: 
        ...
        psi_t = data[:, 0] # column one
        theta = data [:, 1] # column two
        zeta = data[:, 2] # column three
...
# old code was supposedly inserted here as means for erroneous calculations from file paths
...
r_analysis_results = analyser.find_modes(coordinate='R', lambda_width=150.0)    # CHANGING LAMBDA HERE
...
fig.savefig('fourier_transform_r=0.4_lamda=100.png', dpi=300, bbox_inches='tight', pad_inches=0)
plt.show()